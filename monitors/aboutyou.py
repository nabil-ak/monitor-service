...
def discord_webhook(group,sku,store,title, url, thumbnail,prize, sizes, stock):
        """
        ...
        links = {"name": "[ Links ]", 
        "value": f"[CH](https://www.aboutyou.ch/p/nabil/nabil-{sku}) - [CZ](https://www.aboutyou.cz/p/nabil/nabil-{sku}) - [DE](https://www.aboutyou.de/p/nabil/nabil-{sku}) - [FR](https://www.aboutyou.fr/p/nabil/nabil-{sku}) - [IT](https://www.aboutyou.it/p/nabil/nabil-{sku}) - [PL](https://www.aboutyou.pl/p/nabil/nabil-{sku}) - [SK](https://www.aboutyou.sk/p/nabil/nabil-{sku}) - [ES](https://www.aboutyou.es/p/nabil/nabil-{sku}) - [NL](https://www.aboutyou.nl/p/nabil/nabil-{sku})", "inline": False}
        fields.append(links)
...
def checkUpdated(product, store):
    """
...
def comparitor(product, start, groups, store):
    ...
    if available_sizes:
        ping, updated = checkUpdated(product_item, store)
        if updated or start == 1:
            ...
            remove(product_item[2],store)
            
            INSTOCK.append({"Region":store,"Product":product_item})
            if start == 0:
            ...
            if start == 0 and ping:
                for group in groups:
                    ...
                    Thread(target=discord_webhook,args=(
                        group,
                        product['id'],
                        store,
                        product['title'],
...
def monitor(groups,delay=1,keywords=[],proxys=[],blacksku=[]):
    ...
    STORES = [["DE",139],["CH",431],["FR",658],["ES",670],["IT",671],["PL",550],["CZ",554],["SK",586],["NL",545]]

    # Initialising proxy and headers
    proxy_no = 0
    proxy = {} if len(proxys) == 0 else {"http": f"http://{proxys[proxy_no]}"}
    ...
    while True:
        try:
            ...
            if len(proxys) != 0:
                ...
                proxy_no = 0 if proxy_no == (len(proxys) - 1) else proxy_no + 1
                proxy = {"http": f"http://{proxys[proxy_no]}"}
...
if __name__ == '__main__':
    ...
    monitor(groups=[devgroup],proxys=["padifozj-rotate:36cjopf6jt4p@154.13.90.91:80"],keywords=["dunk","jordan 1"],delay=0.1)
